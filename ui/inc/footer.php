<div id="financeurs" class="container has-background-light">
    <h2 class="is-size-3 center">Financement</h2>
    <p class="center">Cet outil a bénéficié d'une évolution grâce à l'appui de :</p>
    <div class="columns">
        <div class="column center">
            <img src="img/financeurs/logo_anct_marianne_100x325.png">
        </div>
        <div class="column center is-one-fifth">
            <img src="img/financeurs/franceeelance_100x100.png">
        </div>
        <div class="column center">
            <img src="img/financeurs/nextgen_80x343.png">
        </div>
    </div>
    <footer>
        <div id="footerWrap">
            <div id="footer" class="content has-text-centered">
                <h3>A propos de cet outil</h3>
                <div class="columns has-text-justified">
                    <div class="column">
                        <p>Cet outil est alimenté avec les données du profil d'acheteur de la salle de marchés de <?php echo gettext("NOM_OPSN")?>.</p>
                        <p>Il a été conçu par le service <b><a href="https://www.ternum-bfc.fr">Données Ouvertes</a></b> de <a href="https://www.ternum-bfc.fr/">ARNia</a> pour faciliter l'exploitation des informations contenues dans les données essentielles de la commande publique.</p>
                    </div>
                    <div class="column">
                        <?php echo gettext('TELECHARGEMENT_DATA')?>
                        <p>Notre page <a href="mentions-legales.php">Mentions légales</a> contient des informations concernant la protection de la vie privée et vous permet de configurer le suivi ou pas par notre outil d'analyse de visites.  </p>
                    </div>
                </div>
                <p><a href=<?php echo gettext('URL_OPSN')?>><img id="footerLogo" src="img/footer_logo.png" alt=<?php echo gettext('NOM_OPSN')?> /></a></p>
            </div>
        </div>
    </footer>


</div>
</html>
