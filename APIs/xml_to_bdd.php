...
-------------------------------
*/
include('getMarches.php');

$token = getToken();

if (!$token)
{
  echo "Nous n'avons pas de token. Stop.";
  die;
}
echo "Nous avons le token $token . On démarre le téléchargement\n\n";

$marches = getMarches($token);

if ($marches)
{
  echo "Données téléchargées. Commence le parsing \n\n";
  $xml = $marches;
}
else
{
